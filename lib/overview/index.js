  ...
  Object.assign(window.parity, {
    rpcOverview: function rpcOverview() {
      ...
      Object.keys(rpc).forEach(function (key) {
        var rpc$ = rpc[key]; // If there are subscribers, then we add

        overview[key] = _objectSpread({}, rpc$.metadata); // We make the `calledWith` field human-readable

        if (rpc$.metadata.calledWith) {
          Object.keys(rpc$.metadata.calledWith).map(function (calledWithKey) {
            var subject$ = rpc$.metadata.calledWith[calledWithKey];
